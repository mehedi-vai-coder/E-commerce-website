    ...
    return productData ? (
        <div className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100 '>
                        ...
                        <div className='flex gap-2'>
                            {
                                productData.sizes.map((item, index) => (
                                    <button onClick={() => setSize(item)} className={`border rounded border-gray-200 py-2 px-4 bg-gray-100 ${item === size ? 'border-orange-500' : ''}`} key={index}>{item}</button>
                                ))}
                        </div>
                    </div>
                    <button onClick={() => addToCart(productData._id, size)} className='bg-black rounded-2xl text-white px-8 py-3 text-sm active:bg-gray-700'>ADD TO CART</button>
            ...
            <div className='mt-20'>
                ...
                {/* Description Box */}
                <div className='flex flex-col gap-4 border border-gray-200 px-6 py-6 text-sm text-gray-500 mt-3 '>
